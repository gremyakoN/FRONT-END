...
export class SearchResultsComponent extends StateComponent implements OnInit, OnDestroy {
    ...
    @Input() showSelectedMembers = true;
    @Input() resultState;
    @Input() paramsState;
    ...
    ngOnInit() {
        this.renderStates([this.columns, this.states.user, this.resultState, this.searchingState, this.states.selectedMembersIDs]);
    ...
    checkboxChanged(element, checked) {
        const arr = this.states.selectedMembersIDs.value;
        if (checked) {
        ...
        this.states.selectedMembersIDs.emit();
    }
